...
class RateLimiter(TimeIndexedStructure):
    ...
    def __init__(self, name: str, total_time: int, in_queue_size: int, hist: SymbolicHistory):
        ...
        self.out = SymbolicHistory(name="{}_out".format(name), total_time=self.total_time)
    ...
    def scheduling_constrs(self) -> List[LabeledExpr]:
        constrs = []
        q = self.queue
        constrs.append(LabeledExpr(q.deqs[0] == 0, "{}_deqs[{}] = {} ".format(self.name, 0, 0)))
        for t in range(1, self.total_time):
            constrs.append(
                LabeledExpr(
                    If(And(q.deqs[t - 1] == 0, q.blog(t) > 0), q.deqs[t] == 1, q.deqs[t] == 0),
                    "{0}_deqs[{1}] = 1 <=> {0}_deqs[{1}-1] == 0".format(self.name, t)
                )
            )
            constrs.append(
                LabeledExpr(
                    If(self.queue.deqs[t] == 1, self.out[t] == self.queue.head_pkt(t), self.out[t] == 0),
                    "{0}_out[{1}] = {0}_q.head({1}) <=> {0}_deqs[{1}] == 1".format(self.name, t)
                )
            )
        return constrs
